...
export default function SingleCube() {
  const coinConfig = {
    rotation: [0.8, 0.05, -0.75] as [number, number, number],
    position: [-5.8, -2, 0] as [number, number, number],
    scale: 8,
  };
  ...
  return (
    ...
    >
      ...
      >
        ...
        {/* 1. MOEDA - Com animação de "Respiração" (Float)
          speed: Velocidade da animação (menor = mais lento/relaxante)
          rotationIntensity: O quanto ele roda sozinho (suave)
          floatIntensity: O quanto ele sobe e desce (amplitude)
        */}
        <Float
          speed={2}
        ...
        >
          <CoinModel
            position={coinConfig.position}
            rotation={coinConfig.rotation}
            scale={coinConfig.scale}
          />
        </Float>
      </Canvas>
    </div>
